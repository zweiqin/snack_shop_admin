...
<!--          <input type="date">-->
          ...
          <Col span="24" v-if="formValidate.is_virtual">
            <FormItem label="虚拟类型：" props="is_virtual">
              <RadioGroup
                ...
                @on-change="changeSpec"
              >
                <Radio :label="1" class="radio">卡密</Radio>
                ...
                <FormItem label="商品属性：" class="labeltop">
                  ...
                  >
                    ...
                    <template slot-scope="{ row, index }" slot="fictitious">
                      ...
                      <Button
                        v-else-if="
                          !row.virtual_list && formValidate.virtual_type == 1
                        "
          ...
          <Col span="24" v-if="formValidate.is_virtual != 1">
        ...
        <div class="scroll-virtual">
          ...
          >
            ...
            <Input
              ...
              placeholder="请输入卡密"
            ></Input>
            <Button type="dashed" @click="removeVirtual(index)">删除</Button>
          </div>
        </div>
        <div class="add-more">
          <Button type="info" @click="handleAdd" icon="md-add">新增</Button>
          <Upload
            class="ml10"
            :action="cardUrl"
            :data="uploadData"
  ...
  methods: {
    ...
    generate(type) {
      generateAttrApi(
        {
          attrs: this.attrs,
          is_virtual: this.formValidate.is_virtual,
        ...
        .then((res) => {
          ...
          if (this.formValidate.is_virtual) {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          this.formValidate.description = this.formatRichText(
      ...
      window.UE.registerUI(
        ...
        function (editor, uiName) {
          ...
          let dialog = new window.UE.ui.Dialog({
            ...
            name: uiName,
            title: "上传图片",
            cssRules: "width:950px;height:543px;padding:16px;",
          });
          this.dialog = dialog;
